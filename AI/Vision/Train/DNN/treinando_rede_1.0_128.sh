...
python2 generate_label.py

echo -e "${Blue}Gerando TFRecords${NC}"
mkdir models
mkdir ./models
python2 generate_tfrecord.py --csv_input=./data/train_labels.csv  --output_path=./models/train.record
...
sed -i "s@PATH_TO_BE_CONFIGURED/model.ckpt@$(pwd)/models/model/mobilenet_v1_1.0_128.ckpt@g" ./models/model/ssd_mobilenet_v1.config
sed -i "s@PATH_TO_BE_CONFIGURED/pet_train.record@$(pwd)/models/train.record@g" ./models/model/ssd_mobilenet_v1.config
sed -i "s@PATH_TO_BE_CONFIGURED/pet_val.record@$(pwd)/models/train.record@g" ./models/model/ssd_mobilenet_v1.config
